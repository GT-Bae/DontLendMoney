...
public class BAKYEONJI_chating : MonoBehaviour
{
    ...
    private const string ProgressKey = "ChatEndIndex_BAKYEONJI"; // 진행 상태 저장 키
    private const string Button1Key = "Button1Clicked_BAKYEONJI";
    private const string Button2Key = "Button2Clicked_BAKYEONJI";
    private const string MoneyKey = "MyMoney"; // 플레이어 돈 저장 키
    private const string HealthKey = "CurrentHealth"; // 체력 저장 키
    private const string BorrowedDayKey_BAKYEONJI = "BorrowedDay_BAKYEONJI"; // 돈 빌린 날짜 키
    ...
    private void Start()
    {
        ...
        // 마지막 진행 상태 확인 및 복원
        int savedEndIndex = PlayerPrefs.GetInt(ProgressKey, 92); // 기본값: 90~92 대화

        // 버튼 상태에 따라 대화 복원
        if (PlayerPrefs.GetInt(Button2Key, 0) == 1) // 버튼 2 선택 시
        {
            StartCoroutine(DisplayChatWithTypingEffect(90, 92, skipTyping: true));
            StartCoroutine(DisplayChatWithTypingEffect(95, 96, skipTyping: true));
        }
        else
        {
            StartCoroutine(DisplayChatWithTypingEffect(90, savedEndIndex, skipTyping: savedEndIndex > 92));
        }
        ...
        // 버튼 클릭 이벤트 등록
        button1.onClick.AddListener(() => OnButtonClicked(button1, 93, 94, true));
        button2.onClick.AddListener(() => OnButtonClicked(button2, 95, 96, false));
    }
    ...
    void SaveBorrowedDay()
    {
        ...
        PlayerPrefs.SetInt(BorrowedDayKey_BAKYEONJI, currentDay);
        PlayerPrefs.Save();
    }

    void CheckRepayment()
    {
        int borrowedDay = PlayerPrefs.GetInt(BorrowedDayKey_BAKYEONJI, -1);
        int currentDay = PlayerPrefs.GetInt(CurrentDayKey);
    ...
    void HandleDebtRepayment()
    {
        ...
        if (randomValue <= 0.7f)
        {
            currentMoney += 7;
            Debug.Log("원금갚음");
            CreateChating(allChatData[97]);
        }
        else if (randomValue <= 0.85f)
        {
            currentMoney += 10;
            Debug.Log("원금+이자까지 갚음");
            CreateChating(allChatData[98]);
        }
        else
        {
            Debug.Log("잠수탔습니다.");
        ...
        PlayerPrefs.SetFloat(MoneyKey, currentMoney);
        PlayerPrefs.DeleteKey(BorrowedDayKey_BAKYEONJI);
        PlayerPrefs.Save();
