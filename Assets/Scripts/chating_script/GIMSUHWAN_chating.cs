...
public class GIMSUHWAN_chating : MonoBehaviour
{
    ...
    private const string ProgressKey = "ChatEndIndex_GIMSUHWAN"; // 진행 상태 저장 키
    private const string Button1Key = "Button1Clicked_GIMSUHWAN";
    private const string Button2Key = "Button2Clicked_GIMSUHWAN";
    private const string MoneyKey = "MyMoney"; // 플레이어 돈 저장 키
    private const string HealthKey = "CurrentHealth"; // 체력 저장 키
    private const string BorrowedDayKey_GIMSUHWAN = "BorrowedDay_GIMSUHWAN"; // 돈 빌린 날짜 키
    ...
    private void Start()
    {
        ...
        // 마지막 진행 상태 확인 및 복원
        int savedEndIndex = PlayerPrefs.GetInt(ProgressKey, 83); // 기본값: 81~83 대화

        // 버튼 상태에 따라 대화 복원
        if (PlayerPrefs.GetInt(Button2Key, 0) == 1) // 버튼 2 선택 시
        {
            StartCoroutine(DisplayChatWithTypingEffect(81, 83, skipTyping: true));
            StartCoroutine(DisplayChatWithTypingEffect(86, 87, skipTyping: true));
        }
        else
        {
            StartCoroutine(DisplayChatWithTypingEffect(81, savedEndIndex, skipTyping: savedEndIndex > 83));
        }
        ...
        // 버튼 클릭 이벤트 등록
        button1.onClick.AddListener(() => OnButtonClicked(button1, 84, 85, true));
        button2.onClick.AddListener(() => OnButtonClicked(button2, 86, 87, false));
    }
    ...
    void SaveBorrowedDay()
    {
        ...
        PlayerPrefs.SetInt(BorrowedDayKey_GIMSUHWAN, currentDay);
        PlayerPrefs.Save();
    }

    void CheckRepayment()
    {
        int borrowedDay = PlayerPrefs.GetInt(BorrowedDayKey_GIMSUHWAN, -1);
        int currentDay = PlayerPrefs.GetInt(CurrentDayKey);
    ...
    void HandleDebtRepayment()
    {
        ...
        if (randomValue <= 0.7f)
        {
            currentMoney += 15;
            Debug.Log("원금갚음");
            CreateChating(allChatData[88]);
        }
        else if (randomValue <= 0.85f)
        {
            currentMoney += 20;
            Debug.Log("원금+이자까지 갚음");
            CreateChating(allChatData[89]);
        }
        else
        {
            Debug.Log("잠수탔습니다.");
        ...
        PlayerPrefs.SetFloat(MoneyKey, currentMoney);
        PlayerPrefs.DeleteKey(BorrowedDayKey_GIMSUHWAN);
        PlayerPrefs.Save();
