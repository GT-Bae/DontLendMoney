...
public class LEEJEONGMAN_chating : MonoBehaviour
{
    ...
    private const string ProgressKey = "ChatEndIndex_LEEJEONGMAN"; // 진행 상태 저장 키
    private const string Button1Key = "Button1Clicked_LEEJEONGMAN";
    private const string Button2Key = "Button2Clicked_LEEJEONGMAN";
    private const string MoneyKey = "MyMoney"; // 플레이어 돈 저장 키
    private const string HealthKey = "CurrentHealth"; // 체력 저장 키
    private const string BorrowedDayKey_LEEJEONGMAN = "BorrowedDay_LEEJEONGMAN"; // 돈 빌린 날짜 키
    ...
    private void Start()
    {
        ...
        // 마지막 진행 상태 확인 및 복원
        int savedEndIndex = PlayerPrefs.GetInt(ProgressKey, 20); // 기본값: 18~20 대화

        // 버튼 상태에 따라 대화 복원
        if (PlayerPrefs.GetInt(Button2Key, 0) == 1) // 버튼 2 선택 시
        {
            StartCoroutine(DisplayChatWithTypingEffect(18, 20, skipTyping: true));
            StartCoroutine(DisplayChatWithTypingEffect(23, 24, skipTyping: true));
        }
        else
        {
            StartCoroutine(DisplayChatWithTypingEffect(18, savedEndIndex, skipTyping: savedEndIndex > 20));
        }
        ...
        // 버튼 클릭 이벤트 등록
        button1.onClick.AddListener(() => OnButtonClicked(button1, 21, 22, true));
        button2.onClick.AddListener(() => OnButtonClicked(button2, 23, 24, false));
    }
    ...
    void SaveBorrowedDay()
    {
        ...
        PlayerPrefs.SetInt(BorrowedDayKey_LEEJEONGMAN, currentDay);
        PlayerPrefs.Save();
    }

    void CheckRepayment()
    {
        int borrowedDay = PlayerPrefs.GetInt(BorrowedDayKey_LEEJEONGMAN, -1);
        int currentDay = PlayerPrefs.GetInt(CurrentDayKey);
    ...
    void HandleDebtRepayment()
    {
        ...
        {
            currentMoney += 50;
            CreateChating(allChatData[25]);
        }
        else if (randomValue <= 0.85f)
        {
            currentMoney += 65;
            CreateChating(allChatData[26]);
        }
        else
        {
            Debug.Log("잠수탔습니다.");
        ...
        PlayerPrefs.SetFloat(MoneyKey, currentMoney);
        PlayerPrefs.DeleteKey(BorrowedDayKey_LEEJEONGMAN);
        PlayerPrefs.Save();
