...
public class GWONJANGMIN_chating : MonoBehaviour
{
    ...
    private const string ProgressKey = "ChatEndIndex_GWONJANGMIN"; // 진행 상태 저장 키
    private const string Button1Key = "Button1Clicked_GWONJANGMIN";
    private const string Button2Key = "Button2Clicked_GWONJANGMIN";
    private const string MoneyKey = "MyMoney"; // 플레이어 돈 저장 키
    private const string HealthKey = "CurrentHealth"; // 체력 저장 키
    private const string BorrowedDayKey_GWONJANGMIN = "BorrowedDay_GWONJANGMIN"; // 돈 빌린 날짜 키
    ...
    private void Start()
    {
        ...
        // 마지막 진행 상태 확인 및 복원
        int savedEndIndex = PlayerPrefs.GetInt(ProgressKey, 56); // 기본값: 54~56 대화

        // 버튼 상태에 따라 대화 복원
        if (PlayerPrefs.GetInt(Button2Key, 0) == 1) // 버튼 2 선택 시
        {
            StartCoroutine(DisplayChatWithTypingEffect(54, 56, skipTyping: true));
            StartCoroutine(DisplayChatWithTypingEffect(59, 60, skipTyping: true));
        }
        else
        {
            StartCoroutine(DisplayChatWithTypingEffect(54, savedEndIndex, skipTyping: savedEndIndex > 56));
        }
        ...
        // 버튼 클릭 이벤트 등록
        button1.onClick.AddListener(() => OnButtonClicked(button1, 57, 58, true));
        button2.onClick.AddListener(() => OnButtonClicked(button2, 59, 60, false));
    }
    ...
    void SaveBorrowedDay()
    {
        ...
        PlayerPrefs.SetInt(BorrowedDayKey_GWONJANGMIN, currentDay);
        PlayerPrefs.Save();
    }

    void CheckRepayment()
    {
        int borrowedDay = PlayerPrefs.GetInt(BorrowedDayKey_GWONJANGMIN, -1);
        int currentDay = PlayerPrefs.GetInt(CurrentDayKey);
    ...
    void HandleDebtRepayment()
    {
        ...
        if (randomValue <= 0.7f)
        {
            currentMoney += 20;
            Debug.Log("원금갚음");
            CreateChating(allChatData[61]);
        }
        else if (randomValue <= 0.85f)
        {
            currentMoney += 25;
            Debug.Log("원금+이자까지 갚음");
            CreateChating(allChatData[62]);
        }
        else
        {
            Debug.Log("잠수탔습니다.");
        ...
        PlayerPrefs.SetFloat(MoneyKey, currentMoney);
        PlayerPrefs.DeleteKey(BorrowedDayKey_GWONJANGMIN);
        PlayerPrefs.Save();
