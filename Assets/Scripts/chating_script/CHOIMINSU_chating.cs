...
public class CHOIMINSU_chating : MonoBehaviour
{
    ...
    private const string ProgressKey = "ChatEndIndex_CHOIMINSU"; // 진행 상태 저장 키
    private const string Button1Key = "Button1Clicked_CHOIMINSU";
    private const string Button2Key = "Button2Clicked_CHOIMINSU";
    private const string MoneyKey = "MyMoney"; // 플레이어 돈 저장 키
    private const string HealthKey = "CurrentHealth"; // 체력 저장 키
    private const string BorrowedDayKey = "BorrowedDay_CHOIMINSU"; // 돈 빌린 날짜 키
    ...
    private void Start()
    {
        ...
        // 마지막 진행 상태 확인 및 복원
        int savedEndIndex = PlayerPrefs.GetInt(ProgressKey, 47); // 기본값: 45~47 대화

        // 버튼 상태에 따라 대화 복원
        if (PlayerPrefs.GetInt(Button2Key, 0) == 1) // 버튼 2 선택 시
        {
            StartCoroutine(DisplayChatWithTypingEffect(45, 47, skipTyping: true));
            StartCoroutine(DisplayChatWithTypingEffect(50, 51, skipTyping: true));
        }
        else
        {
            StartCoroutine(DisplayChatWithTypingEffect(45, savedEndIndex, skipTyping: savedEndIndex > 47));
        }
        ...
        // 버튼 클릭 이벤트 등록
        button1.onClick.AddListener(() => OnButtonClicked(button1, 48, 49, true));
        button2.onClick.AddListener(() => OnButtonClicked(button2, 50, 51, false));
    }
    ...
    void HandleDebtRepayment()
    {
        ...
        {
            currentMoney += 10;
            CreateChating(allChatData[52]);
        }
        else if (randomValue <= 0.85f)
        {
            currentMoney += 14;
            CreateChating(allChatData[53]);
        }
        else
        {
            Debug.Log("잠수탔습니다.");
